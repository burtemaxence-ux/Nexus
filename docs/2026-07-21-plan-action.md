...
| P0-2 | ~~Preuve sociale fabriquée~~ | ✅ **fait le 21/07, périmètre élargi** : bandeau → secteurs (plus de noms fictifs ni « O'Tacos ») ; stats inventées (« 1 200+ établissements », « 0 contentieux depuis 2023 », « 2 300 € ») → 4 faits vérifiables du produit ; **6 faux avis clients retirés** (Art. L121-4 : pratique réputée trompeuse en toutes circonstances) — le composant reste prêt à rebrancher avec de vrais témoignages (G-1/G-3) | — | ✅ fait | plus aucune preuve sociale inventée sur la landing ✅ |
...
|----|--------|-------------------------|--------|-------------------|
| P1-1 | ~~KPI « ce planning me coûte X € »~~ | ✅ **fait le 21/07** : footer de la carte « Heures planifiées » = `≈ X € · ±Y% vs N-1` (heures × `hourly_cost` du poste) ; retombe sur le % d'heures si aucun coût horaire configuré | — | ✅ typecheck/build verts — **à valider visuellement avec des coûts horaires renseignés** |
...
| P1-3 | ~~Révoquer l'EXECUTE anon~~ | ✅ **fait le 21/07** — migration 083 appliquée en prod ; smoke public + redirection auth vérifiés après coup | — | ✅ advisor anon fermé |
| P1-4 | ~~Smoke en CI~~ | ✅ **fait le 21/07 (version HTTP)** : `scripts/smoke.mjs` (landing, login, register, page SEO, manifest, redirection route protégée) vérifié en local contre l'app buildée, branché en CI (étape sautée tant que les 2 variables publiques Supabase ne sont pas dans GitHub → Actions → Variables). Le parcours Playwright authentifié (login → shift → publier) reste ouvert : il nécessite un compte de test dédié — à créer par Maxence | — | ✅ smoke local 6/6 ; CI prête (variables à ajouter) |
| P1-5 | ~~Typo : Syne retirée~~ | ✅ **fait le 21/07** — option (b), conforme à la reco de l'audit de juin (grotesque sobre) : 72 usages `var(--font-syne)` → Manrope (graisse 800 ajoutée), Syne retirée du layout. 3 familles chargées. **Réversible en 1 commit si le rendu des titres ne convient pas** | — | ✅ ≤ 3 familles ; build vert |
...
| P1-7 | Restauration de backup | ✅ **procédure écrite le 21/07** (`docs/deployment.md` §10 : restauration complète, chirurgicale, quoi sauvegarder hors DB). ⏳ **Le test réel reste à faire par Maxence** (projet jetable = dashboard + facturation) — tableau de chronométrage prêt dans la doc | 1 h | test trimestriel effectué et daté dans le tableau |
