...
${candidateDescriptions}`,
  ...
  if (rrErr || !replacementRequest) {
    ...
    // On la récupère et on la retourne telle quelle
    if (rrErr?.code === '23505') {
      ...
      if (existingRR) {
        const existingCandidateIds: string[] = (existingRR.candidates as { employee_id: string }[]).map(c => c.employee_id)
        const matchedCandidates = top3.filter(c => existingCandidateIds.includes(c.employee_id))
        const finalCandidates = matchedCandidates.length > 0 ? matchedCandidates : top3
