...
// Récit en blocs (chaînes JS, apostrophes simples). Aucun tiret nulle part.
const HISTOIRE = [
...
const HISTOIRE_SUITE =
  "Il aurait pu en rester là. Mais d'autres patrons ont voulu le même outil. Le petit script est devenu Quartzbase. Pas pour changer le monde, juste pour régler un problème que des milliers de gérants vivent chaque semaine. Si vous tenez une boulangerie ou un restaurant, il a été pensé pour vous, par quelqu'un qui a vu de près ce que ça coûte de le faire à la main."

const TEAM_INTRO = "Pour l'instant, Quartzbase, c'est surtout une personne."
const TEAM_ROLE = "Fondateur. Celui qui code, et celui qui vous répond."
const TEAM_P =
  "C'est lui qui construit l'outil. C'est aussi lui qui répond quand vous écrivez. Pas de centre d'appel, pas de numéro de ticket à rallonge. La personne au bout du message est celle qui a fait le produit, et qui connaît chaque établissement qui l'utilise. Une petite équipe, ça fait sans doute moins sérieux qu'un grand logo. On préfère l'assumer. Comme ça, vous savez exactement à qui vous confiez votre planning."
  ...
  return (
    <PageWrap maxWidth={860}>
      <PageHeader
        ...
        intro="Pas une grande boîte anonyme. Juste un fondateur de 19 ans, et un outil né pour régler un problème bien réel."
      />
      ...
      <Section title="Notre histoire">
        <div style={{ maxWidth: 680 }}>
          {HISTOIRE.map((p, i) => (
            <p key={i} style={{ fontSize: 17.5, color: '#c2c2d4', lineHeight: 1.8, margin: '0 0 20px' }}>{p}</p>
          ))}
      ...
      <Section title="L’équipe">
        <p style={{ fontSize: 16, color: '#a6a8b8', lineHeight: 1.7, margin: '0 0 22px', maxWidth: 620 }}>{TEAM_INTRO}</p>
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap', background: '#111118', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '26px 28px' }}>
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            ...
            <div style={{ fontSize: 13.5, color: '#8b86ff', marginBottom: 12 }}>{TEAM_ROLE}</div>
            <p style={{ fontSize: 15, color: '#a6a8b8', lineHeight: 1.7, margin: 0 }}>{TEAM_P}</p>
          </div>
        </div>
      </Section>
