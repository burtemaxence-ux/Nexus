...
function AlgoCount({ n }: { n: number }) {
  if (!n) return null
  return (
    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--text-tertiary)]" title="Plannings générés par l'algorithme ce mois-ci">
      · {n} algo
    </span>
  )
}
    ...
    return (
      <span className="inline-flex items-center gap-1.5">
        ...
        >
          ...
          Quota IA atteint — Passer au plan Pro →
        </Link>
        <AlgoCount n={quota.algoUsed ?? 0} />
      </span>
    )
    ...
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-medium text-[var(--text-secondary)]">
          {quota.used}/{quota.limit} générations IA
        </span>
      </div>
